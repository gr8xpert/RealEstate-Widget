...
// Get branding from URL params
$companyName = isset($_GET['companyName']) ? htmlspecialchars(urldecode($_GET['companyName'])) : '';
$logoUrl = isset($_GET['logoUrl']) ? filter_var(urldecode($_GET['logoUrl']), FILTER_SANITIZE_URL) : '';
...
<head>
    <meta charset="UTF-8">
    <title>' . ($companyName ? $companyName . ' - ' : '') . 'My Wishlist</title>
    <style>
        body {
...
<body>
    <div class="header">
        ' . ($logoUrl ? '<img src="' . $logoUrl . '" alt="' . $companyName . '" class="header-logo"><br>' : '') . '
        ' . ($companyName ? '<div class="header-company">' . $companyName . '</div>' : '') . '
...
$html .= '
    <div class="footer">
        <p>' . ($companyName ? 'Generated by ' . $companyName : 'Generated by RealtySoft Widget') . '</p>
        ' . ($websiteUrl ? '<p>' . preg_replace('/^https?:\/\//', '', $websiteUrl) . '</p>' : '') . '
    </div>
</body>
</html>
';
