/**
 * RealtySoft Widget v2 - Property Carousel Component
...
class RSPropertyCarousel extends RSBaseComponent {
    ...
    init() {
        ...
        this.active = 0; // Will be set properly after properties load
        this.isAnimating = false;
        this.v2Initialized = false; // Track if V2 items have been created
        this.v3Initialized = false; // Track if V3 items have been created
        ...
        this.autoPlayInterval = parseInt(this.element.dataset.rsInterval) || 5000;
        this.limit = parseInt(this.element.dataset.rsLimit) || 10;
    ...
    generatePropertyUrl(property) {
        ...
        const pageSlug = (typeof RealtySoftState !== 'undefined' && RealtySoftState.get('config.propertyPageSlug')) || 'property';
    ...
    bindEvents() {
        ...
        if (this.dotsContainer) {
            this.dotsContainer.addEventListener('click', (e) => {
                const dot = e.target.closest('.rs-property-carousel__dot');
                if (dot) {
        ...
        this.itemsContainer.addEventListener('click', (e) => {
            const item = e.target.closest('.rs-property-carousel__item');
            if (!item) return;
    ...
    renderItemsV2() {
        ...
        items.forEach((item, index) => {
            ...
            this.updateCarouselItemV2Transform(item, {
                offsetFromActive,
                absOffsetFromActive,
                isActive,
                isVisible
    ...
    createCarouselItemV2(property, index) {
    ...
    updateCarouselItemV2Transform(item, { offsetFromActive, absOffsetFromActive, isActive, isVisible }) {
        ...
        item.style.zIndex = 10 - absOffsetFromActive;
        item.style.opacity = isVisible ? (isActive ? '1' : '0.95') : '0';

        // Update overlay opacity
        const overlay = item.querySelector('.rs-property-carousel__card-overlay');
        if (overlay) {
    ...
    renderItemsV3() {
        ...
        items.forEach((item, index) => {
            ...
            this.updateCarouselItemV3Transform(item, {
                offsetFromActive,
                absOffsetFromActive,
                isActive,
                isVisible
    ...
    createCarouselItemV3(property, index) {
        const item = document.createElement('div');
        item.className = 'rs-property-carousel__item';
        item.dataset.index = index;
    ...
    updateCarouselItemV3Transform(item, { offsetFromActive, absOffsetFromActive, isActive, isVisible }) {
        // Get responsive values based on screen width
        const screenWidth = window.innerWidth;
        let translateXMultiplier = 240; // Wider spread between cards
        let rotateMultiplier = 45; // Rotation angle for side cards
        let scaleBase = 0.85;
        let zDepth = 100; // How far back side cards go
        ...
        // Left cards (offsetFromActive > 0) rotate positive (right edge back)
        // Right cards (offsetFromActive < 0) rotate negative (left edge back)
     ...
     * Large image with prev/next panels on sides
     */
    renderItemsV4() {
    ...
    createCarouselItemV4(property, index) {
        const item = document.createElement('div');
        item.className = 'rs-property-carousel__item';
        item.dataset.index = index;
    ...
    updatePageCounter() {
        ...
        let counter = this.element.querySelector('.rs-property-carousel__counter');
        if (!counter) {
     ...
     * Shows 3 tall images with parallelogram shape
     */
    renderItemsV5() {
    ...
    createCarouselItemV5(property, index) {
        const item = document.createElement('div');
        item.className = 'rs-property-carousel__item';
        item.dataset.index = index;
     ...
     * Shows 4 cards with large numbers, title, image, price, specs
     */
    renderItemsV6() {
    ...
    createCarouselItemV6(property, index, displayNumber, isActive) {
        const item = document.createElement('div');
        ...
        const location = property.location?.name || property.location || property.area || property.city || '';
        ...
        const viewDetailLabel = this.label('view_detail') || 'View Detail';
        ...
        item.innerHTML = `
            ...
            ${location ? `<p class="rs-property-carousel__card-location">${this.escapeHtml(location)}</p>` : ''}
            <a href="${propertyUrl}" class="rs-property-carousel__card">
            ...
            <button class="rs-property-carousel__card-view-btn" onclick="window.location.href='${propertyUrl}'">${viewDetailLabel}</button>
        `;

        return item;
    }

    createCarouselItem(property, level, index) {
        const item = document.createElement('div');
...
// Register component
RealtySoft.registerComponent('rs_property_carousel', RSPropertyCarousel);
